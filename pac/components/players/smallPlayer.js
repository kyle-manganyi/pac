...
class PlaylistItem {
  constructor(name, uri, isVideo, artist, id) {
    this.name = name;
    this.uri = uri;
...
export default class App extends React.Component {
  ...
  async componentDidUpdate(){
    let temp = [];
    let downloads = []
     await AsyncStorage.getItem("nowPlaying").then(value => {
        if (value) {
            downloads = [JSON.parse(value)];
        }
      });

      if(downloads[0].id !== this.state.PLAYLIST[0].id){
        await downloads.forEach(element => {
            temp.push(new PlaylistItem(element.title,element.audio,false,element.artist, element.id)
            )      
        });
  ...
  render() {
    return (
      <View style={{flexDirection:"row", justifyContent:"space-between", marginHorizontal:"5%", zIndex:999999}}>
          ...
          <View style={{flexDirection:"row", justifyContent:"space-between", width:"30%"}}>
              <Text ellipsizeMode="tail" numberOfLines={1} style={{fontSize:12, maxWidth:100}}>{this.state.PLAYLIST.length > 0 ?  this.state.PLAYLIST[this.index].name : null}</Text>
              <Text>-</Text>
              <Text ellipsizeMode="tail" numberOfLines={1} style={{fontSize:12, maxWidth:100}}>{this.state.PLAYLIST.length > 0 ?  this.state.PLAYLIST[this.index].artist : null}</Text>
          </View>
